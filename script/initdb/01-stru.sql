...
(
    ...
    serial_number     varchar(255)
        constraint uk4ir8qowu1sbtfeh2lbrfjj62y
            unique,
    status            varchar(255)
        constraint product_instances_status_check
            check ((status)::text = ANY
                   ((ARRAY ['CREATED'::character varying, 'IN_STOCK'::character varying, 'SOLD'::character varying])::text[])),
    updated_at        timestamp(6) with time zone,
    product_id        bigint
        constraint fk72d9n5tidov45n7kidh9a0cfj
            references products
);
...
(
    ...
    product_instance_id bigint,
    product_type        varchar(255)
        constraint sale_order_items_product_type_check
            check ((product_type)::text = ANY
                   ((ARRAY ['MATERIAL'::character varying, 'ASSET'::character varying])::text[])),
    quantity            integer          not null,
    unit_price          double precision not null,
